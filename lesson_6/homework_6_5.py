...
class Pen(Stationary):
    ...
    def draw(self):
        return f'Вы взяли ручку "{self.title}".'
...
class Pencil(Stationary):
    ...
    def draw(self):
        return f'Вы взяли карандаш "{self.title}".'
...
class Handle(Stationary):
    ...
    def draw(self):
        return f'Вы взяли маркер "{self.title}".'


pen = Pen('Ручка красная')
pencil = Pencil('Карандаш черный')
handle = Handle('Маркер зеленый')
print(pen.draw())
print(pencil.draw())
print(handle.draw())
